...
router.get("/memberAddCon", function (req, res) {
    ...
    var info = {bumenid, zhiwu, qq, email, name, phone, sex,pass, state, numbers};
...
router.get("/show",function (req,res) {
    mysql.query("select * from member",function (err,result) {
        if(err){
            ...
            res.render("admin/memberShow",{info:result});
        }
    })
})
...
router.get("/edit/:id",function (req,res) {
    ...
    // select bumen.name as bname,member.* from bumen,member where bumen.id=member.bumenid
    mysql.query("select * from member where id=" + id, function (err, result) {
        if (err) {
            console.log("err");
        } else {
            res.render("admin/memberEdit", {info: result});
        }
    })
})
